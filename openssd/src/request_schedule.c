...
void IssueNandReq(unsigned int chNo, unsigned int wayNo)
{
	...
	else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ_TRANSFER)
	{
		...
		if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.nandEcc == REQ_OPT_NAND_ECC_ON){
			...
			//if (dieStateTablePtr->dieState[chNo][wayNo].reserved++ == 0 && nvme_init_done == 1){
			//if (nvme_init_done == 1) {
				//XTime_GetTime(&(dieStateTablePtr->dieState[chNo][wayNo].tstart));
				//xil_printf("[%lu][%lu] READ PAGE, cnt: %lu\r\n", chNo, wayNo, dieStateTablePtr->dieState[chNo][wayNo].reserved);
			//}
		}
		else {
...
unsigned int CheckReqStatus(unsigned int chNo, unsigned int wayNo)
{
	...
	if(dieStateTablePtr->dieState[chNo][wayNo].reqStatusCheckOpt == REQ_STATUS_CHECK_OPT_COMPLETION_FLAG)
	{
		...
		if(V2FTransferComplete(completeFlag))
		{
			...
			//if (nvme_init_done == 1) {
			//	XTime_GetTime(&(dieStateTablePtr->dieState[chNo][wayNo].tend));
			//	xil_printf("[%lu][%lu] READ PAGE: %llu\r\n", chNo, wayNo, dieStateTablePtr->dieState[chNo][wayNo].tend - dieStateTablePtr->dieState[chNo][wayNo].tstart);
				//dieStateTablePtr->dieState[chNo][wayNo].reserved = 0;
			//}
