...
// Write Log for CXL-SSD (default: 64MB)
#define WRITE_LOG_START_ADDR (RESERVED0_START_ADDR)
#define WRITE_LOG_SIZE (sizeof(WriteLogEntry) * MAX_LOG_ENTRIES)

// Log Compaction Buffer for CXL-SSD (default: 8MB)
#define LOG_COMPACT_BUFFER_COUNT (1)
#define LOG_COMPACT_BUFFER_START_BEFORE_ALIGN (WRITE_LOG_START_ADDR + WRITE_LOG_SIZE)
#define LOG_COMPACT_BUFFER_START (LOG_COMPACT_BUFFER_START_BEFORE_ALIGN + (16384 - LOG_COMPACT_BUFFER_START_BEFORE_ALIGN % 16384))
#define LOG_COMPACT_BUFFER_SIZE (BYTES_PER_DATA_REGION_OF_SLICE * LOG_COMPACT_BUFFER_COUNT)
#define LOG_COMPACT_BUFFER_END (LOG_COMPACT_BUFFER_START + LOG_COMPACT_BUFFER_SIZE)

//cacheline read location (64 bytes)
#define DUMMY_CACHE_READ LOG_COMPACT_BUFFER_END + 1
...
// Uncached & Unbuffered
// for data buffer
#define TEMPORARY_DATA_BUFFER_BASE_ADDR 0x10000000
#define TEMPORARY_SPARE_DATA_BUFFER_BASE_ADDR (TEMPORARY_DATA_BUFFER_BASE_ADDR + AVAILABLE_TEMPORARY_DATA_BUFFER_ENTRY_COUNT * BYTES_PER_SPARE_REGION_OF_SLICE)
...
#define RESERVED1_START_ADDR (FTL_MANAGEMENT_END_ADDR + 1) //~0x201A8820
// Log Index for CXL-SSD (default: 256MB)
#define LOG_INDEX_START_ADDR (RESERVED1_START_ADDR)
#define LOG_INDEX_SIZE (sizeof(uint32_t) * LOG_INDEX_BUCKET_COUNT * LOG_INDEX_BUCKET_SIZE)
// 512KB buffer for DMA-based memory trace pushing for CXL-SSD
// #define TRACE_DMA_BUFFER_START_BEFORE_ALIGN (RESERVED1_START_ADDR)
// #define TRACE_DMA_BUFFER_START  (TRACE_DMA_BUFFER_START_BEFORE_ALIGN + (4096 - TRACE_DMA_BUFFER_START_BEFORE_ALIGN % 4096))
// #define TRACE_DMA_BUFFER_SIZE   (512 * 1024) // MAX DMA SIZE (512KB)
// #define TRACE_DMA_BUFFER_END    (TRACE_DMA_BUFFER_START + TRACE_DMA_BUFFER_SIZE)

// NAND page list for Log Compaction
#define NAND_PAGE_LIST_START (LOG_INDEX_START_ADDR + LOG_INDEX_SIZE)
#define NAND_PAGE_LIST_SIZE (LOG_INDEX_BUCKET_COUNT * sizeof(uint8_t))
#define NAND_PAGE_LIST_END (NAND_PAGE_LIST_START + NAND_PAGE_LIST_SIZE)

#define DATA_BUFFER_BASE_ADDR NAND_PAGE_LIST_END
